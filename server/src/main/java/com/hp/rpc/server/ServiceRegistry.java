...
public class ServiceRegistry implements BeanPostProcessor, Closeable {
	...
	public void init() throws Exception {
		...
		if (StringUtils.isEmpty(serverConfigBean.getZkRootPath())) {
			serverConfigBean.setZkRootPath(RPCConstant.ZK_ROOT_PATH);
		}
		if (serverConfigBean.getZkSessionTimeout() == 0) {
			serverConfigBean.setZkSessionTimeout(RPCConstant.ZK_SESSION_TIMEOUT);
		}
		
		//初始化zk
		if (zooKeeper == null) {
			zooKeeper = ZookeeperHelper.getConnection(serverConfigBean.getZkAddress(), serverConfigBean.getZkSessionTimeout());
		}
		
		//创建zk根节点
		ZookeeperHelper.createNode(zooKeeper, serverConfigBean.getZkRootPath(), null, CreateMode.PERSISTENT);
	...
	private void registry(Class<?> clazz, String beanName) {
		...
		for (Method m : methods) {
			...
			try {
				//创建服务节点
				ZookeeperHelper.createNode(zooKeeper, serverConfigBean.getZkRootPath() + "/" + clazz.getName() + "." + m.getName(), null, CreateMode.PERSISTENT);
				
				//创建服务数据
				String uuid = UUID.randomUUID().toString();
				RegisterBean register = new RegisterBean();
				ZookeeperHelper.createNode(zooKeeper, serverConfigBean.getZkRootPath() + "/" + clazz.getName() + "." + m.getName() + "/" + uuid, register.toString(), CreateMode.EPHEMERAL);
			} catch (Exception e) {
	...
	private boolean checkMethod(String className, String methodName) {
		...
		//判断该方法是否在允许的范围内
		if (CollectionUtils.isNotEmpty(serverConfigBean.getAllowClassMethods()) && serverConfigBean.getAllowClassMethods().contains(className + "." + methodName)) {
			return true;
		}
